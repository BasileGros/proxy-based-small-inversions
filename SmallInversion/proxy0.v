...
Definition create_call_constructor
  ...
  {| bcontext := map decl_name cstr.(cstr_args);
      bbody := tApp (tVar (cstr.(cstr_name))) (calls_params ++ call_args)
  |}.
...
Definition create_proxy0 (transfo_info : transformation_info) : ErrorMonad term :=
  ...
  let proxy_type0 := tApp (tVar (name_disp)) calls_params_indices in

  (*The name and context declaration of the object to invert.*)
  let name_rel' :=
    {| binder_name := nNamed ((poib transfo_info).(pseudo_name) ^ "_r'"); binder_relevance := Relevant |}
  in

  let decl_rel :=
    {|
      decl_name := string_to_aname ((poib transfo_info).(pseudo_name) ^ "_r");
      decl_body := None;
  ...
  let proxy_body :=
    tCase
      ...
      {|
        ...
        preturn := tApp (tVar (name_disp)) (calls_params ++ calls_indices)
      |}(tVar ((poib transfo_info).(pseudo_name) ^ "_r"))
...
Definition create_proxy0 (transfo_info : transformation_info) : ErrorMonad (term * term) :=
  
  (*Name and context of the inductive to proxy/the proxy of the inductive type to feed to proxy0*)
  let name_disp := (poib transfo_info).(pseudo_name) in
  let name_inverter := (poib transfo_info).(pseudo_name) ^ "_inverter" in
  let decl_disp := vass (string_to_aname name_disp) (poib transfo_info).(pseudo_type) in                                                  
  (*Name the contexts of the inductive for the future variable uses. *)
  let ctx_constructors :=
    rev (map (create_context_constructor name_disp) (concat_options (lctors transfo_info)))
  ...
  let calls_params_db :=
    if isdep transfo_info
    then
      rev_range_deBruijn (length calls_params) (length (poib transfo_info).(pseudo_indices))
    else
      rev_range_deBruijn (length calls_params) (1 + length (poib transfo_info).(pseudo_indices))
  in
  let type_inverter :=
    (context_to_tProd (rev ctx_params_indices))
      (tProd (nameAnon) (tApp (term_og_inductive transfo_info) (rev_range_deBruijn (length ctx_params_indices) 0))
         (tApp (tVar name_disp) (calls_params_db ++ calls_indices))) in
  let decl_inverter := vass (string_to_aname name_inverter) type_inverter in

  let ctx_instance := [decl_inverter; decl_disp] in
...
and transforming the variabel references into De Bruijn indexes.*)
  ...
  let typeclass_proxy :=
    ...
    then
      ...
      {| inductive_mind := (MPfile ["typeclass"; "SmallInversion"], "InvProxy");
        inductive_ind := 0 |}
  in
  let proxy0_instance := tApp (tVar (name_inverter)) calls_params_indices in
  let instance_proxy := tApp
                          (tConstruct typeclass_proxy 0 [])
                          [orig_type;proxy_type0;proxy0_instance]
  in

  (*Adds the lambda terms of the parameters and indices of the inductive and converts the corresponding variables into deBruijn indices.*)
  let lambda_proxy0 :=
    append_context_vars_lambda (env_quote transfo_info) ctx_params_indices proxy0
  in
  (*Adds the lambda terms of the type and constructors for the composition of the strategy results.*)
  let term_proxy0 :=
    append_context_vars_lambda (env_quote transfo_info) ctx_constructors lambda_proxy0
  in
  ...
  in
  let term_instance :=
    append_context_vars_lambda (env_quote transfo_info) ctx_instance lambda_instance
  in
  Success (term_instance, term_proxy0 ) .
