...
(*The closed lambda terms for the proxy_type and each branch of the the proxy function.
 ...
 Used as a result of a transformation.*)
Definition proxy_adapter : Type := term * list (option term) %type.

(*The partially composed together closed lambda terms for the proxy_type and each branch of the the proxy function.
 Used when performing the composition of all results.*)
Definition result_proxy : Type := term * list (option term) %type.

(*The datatype to encode objects to define in the global Rocq environment once the transformations and compositions are finishes*)
...
(*An accumulator of global definitions.*)
Definition glob_diff := list Glob_def -> list Glob_def.

(*The definition of a transformation : takes in all infos necessary, returns the proxy type and function, the list of new inductives, and the accumulator of side global definitions.*)
Definition transformation := transformation_info -> ErrorMonad (proxy_adapter * list transformation_info * glob_diff)%type.

(*The result of compositions: the proxy type, the branches of the proxy function and the global definitions of the partial inductive types and proxies*)
Definition Result := result_proxy * glob_diff%type.
(* type family + generator functions *)

(*Flags used in strategies to indicate whether or not to accumulate side information.
 Used with TmDebugStrategy*)
...
Fixpoint map_and_merge_results {A} (lf: list (A -> ErrorMonad Result)) (lA:list A)
  (m : list (result_proxy*glob_diff) -> ErrorMonad Result)
  : ErrorMonad Result :=
  match lf, lA with
  | _, nil => m nil
...
Fixpoint debug_map_and_merge_results {A} (lf: list (A -> ErrorMonad Result)) (lA:list A)
  (m : list A ->  list (result_proxy*glob_diff) -> ErrorMonad Result)
  : ErrorMonad Result :=
  match lf, lA with
  | _, nil => m nil nil
  | nil, _ => Error "When merging results, not enough strategy continuations."
...
Fixpoint execute_strategy
  ...
  : ErrorMonad Result :=
      match strat with
      |strat_Finish => Error "Strategy Failed : execution finished."
      |strat_Orelse s1 s2 =>
